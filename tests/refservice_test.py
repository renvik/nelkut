	...
	def test_add_book_to_database(self):
		...
		with self.app.app_context():
			...
			request = MockupRequest({"cite_id": "abcdefghijk", 
									 ...
									 "end_page": 2,
									 "user_id": 1})
			refservice.add_book_to_database(self.db, request, "test_user")
	...
	def test_add_article_to_database(self):
		...
		with self.app.app_context():
			...
			request = MockupRequest({"cite_id": "smfokmsc", 
									...
									"end_page": 4,
									"user_id": 1})
			refservice.add_article_to_database(self.db, request, "test_user")
	...
	def test_add_inproceeding_to_database(self):
		...
		with self.app.app_context():
			...
			request = MockupRequest({"cite_id": "ldjfajsja", 
									...
									"end_page": 3,
									"user_id": 1})
			refservice.add_inproceeding_to_database(self.db, request, "test_user")
